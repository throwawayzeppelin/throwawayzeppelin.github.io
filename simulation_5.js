...
function setupInputListeners() {
    ...
    document.getElementById('rotateotherdir').addEventListener('click', () => {
        if (model && model_container && spiral) {
            ...
            model.position.setX(48);
            model_container.position.setX(10);

            // Make spiral visible, set its position, and 
          //
          // scale it
            spiral.visible = true;
            spiral.position.set(49, -5, -20);
            spiral.scale.set(1.7, 1.7, 1.7);  // Scale up by 20%

            // Rotate spiral for 1 second
            ...
            let animate = () => {
                ...
                if (elapsedTime < 1000) {
                    ...
                    spiral.scale.set(1, 1, 1);  // Reset scale
                    rotationDirection *= -1;
                    
                    // Return model and model_container to their original positions
    ...
    document.getElementById('cleaningjet').addEventListener('click', () => {
        ...
        console.log('Thingy position:', thingy.position);
        console.log('Model position:', model.position);
    });
    
    document.getElementById('vizualize').addEventListener('click', () => {
        console.log('Thingy position:', thingy.position);
        console.log('Model position:', model.position);
        visualizeIce();
    });
...
async function init() {
    ...
    try {
        ...
        if (model) {
            model.position.set(30, -17, -20);
            model.scale.set(0.7, 1.8, 0.7);
            thingy.position.set(10, 0, -2); 
        }
